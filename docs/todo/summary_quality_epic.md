...
- [ ] Refactor `summarizer_agent.py` to use medical reporting standards (e.g., CONSORT or STROBE).
...
- [ ] Add a prompt instruction to SummarizerAgent to distinguish between statistically significant and clinically meaningful.
...
- [ ] Update `reviewer_agent.py` to simulate a picky medical journal editor.
...
- [ ] Create a service that generates alt-text for every chart that explains the trend to a non-expert (e.g., "This boxplot shows that the median recovery time for the Drug group is 2 days shorter than the Placebo group").
