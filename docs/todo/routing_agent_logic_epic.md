...
- [ ] Implementation guide:
  - Step 1: Extend `DataBlueprint` with `is_paired` and `group_samples` dict.
  - Step 2: Update `registry.py` metadata with the new constraints.
  - Step 3: Split `ComparisonAgent` into `PairedComparisonHandler` and `IndependentComparisonHandler` nodes.
  - Step 4: Make `edges.py` raise `RoutingError` when an independent-only node is suggested for paired data.
