...
- [ ] Calculate Cohen's d for t-tests (paired/independent) and include 95% confidence intervals in the return payload.
- [ ] Calculate partial eta squared for ANOVA (parametric and non-parametric analogs).
- [ ] Add post-hoc power analysis to interpret non-significant results (determine if the study was underpowered).
- [ ] Standardize `confidence_interval` fields across comparison/anova outputs.

## Core: Multi-Level & Regression
- [ ] Create `statmate/statistical_core/regression.py` with regression helpers.
- [ ] Implement simple linear regression (beyond correlation) to provide coefficients and intercepts.
- [ ] Add multiple linear regression support with collinearity checks (VIF) and flag high-VIF columns.
- [ ] Implement basic logistic regression for binary outcome prediction with model fit diagnostics.

## Data Quality: Outlier & Influence Detection
- [ ] Implement Cook’s distance and leverage calculation for regression tasks.
- [ ] Add Tukey’s fences or z-score-based outlier detection in the initial insights phase and feed into assumption diagnostics.
