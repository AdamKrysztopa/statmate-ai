...
def normality_of_difference_agent(
    ...
    return build_stat_test_agent(
        ...
        test_name=test_name,
        test_function=test_function,
        difference_with_standard_normality=(
            'Tests if difference between the two samples is drawn from a normal distribution.'
        ),
    ...
    def run_meta(ctx: RunContext[StatTestDeps]) -> tuple[StatTestResult, str]:
        ...
        for agent_fn, weight in agents:
            ...
            rejected = (
                ...
                else all(p < alpha for p in result.statistical_test_result.p_value)  # type: ignore
