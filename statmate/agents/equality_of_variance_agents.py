...
def bartlett_agent(
    ...
    return build_stat_test_agent(
        ...
        potential_suggestions=(
            "Remember Bartlett's test is sensitive to non-normality; "
            "if your data depart from normality, consider Levene's or Brown–Forsythe tests."
            ...
            "Apply variance-stabilizing transformations (e.g., log, square-root, or Box–Cox) "
            "if variances increase with the mean."
            "For multiple groups beyond two, use Bartlett's test extension "
            "or consider ANOVA on absolute deviations (Brown–Forsythe)."
        ),
    )
...
def levene_agent(
    ...
    return build_stat_test_agent(
        ...
        test_function=test_function,
        potential_suggestions=(
            "Levene's test uses the median (default) for robustness; "
            "use `center='mean'` if you assume symmetric distributions."
            ...
            "Fligner–Killeen test is another nonparametric option less sensitive to outliers; "
            "consider it when Levene's assumptions fail."
        ),
    )
