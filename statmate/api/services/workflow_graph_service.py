...
def render_workflow_graph(graph_state: dict[str, Any]) -> dict[str, Any]:
    ...
    svg_parts: list[str] = [
        ...
        '<stop offset="100%" stop-color="#0e1c33"/></linearGradient>',
    ...
    for edge in edges:
        ...
        svg_parts.append(
            f'<line x1="{x1 + 90}" y1="{y1}" x2="{x2 - 50}" y2="{y2}"'
            f' stroke="{color}" stroke-width="2.5" marker-end="url(#arrow)" opacity="0.85" />'
        )
    ...
    for node in nodes:
        ...
        svg_parts.append(
            f'<g><rect x="{x - 28}" y="{y - 36}" rx="14" ry="14"'
            f' width="{radius + 56}" height="72" fill="{fill}" stroke="{stroke}"'
            f' stroke-width="2.5" opacity="0.96" />'
            f'<text x="{x + radius / 2}" y="{y - 4}" dominant-baseline="middle"'
            f' text-anchor="middle" fill="#e2e8f0"'
